...
func (s *ScraperService) createOrGetRecord(tx *gorm.DB, listing scraper.ParsedListing) (*models.Record, error) {
	...
	// Use raw SQL to insert without timestamp fields
	sql := `INSERT INTO discogs_record (discogs_id, artist, title, format, label, catno, wants, haves, added, genres, styles, suggested_price, year) 
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING id`
	
	var recordID uint
	err := tx.Raw(sql, 
		record.DiscogsID, record.Artist, record.Title, record.Format, record.Label, 
		record.Catno, record.Wants, record.Haves, record.Added, record.Genres, 
		record.Styles, record.SuggestedPrice, record.Year).Scan(&recordID).Error
