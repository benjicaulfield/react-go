...
func (h *Handler) SearchSellerListings(c *gin.Context) {
	...
	if err := h.db.Where("name ILIKE ?", "%"+req.Seller+"%").First(&seller).Error; err != nil {
		c.JSON(http.StatusOK, []gin.H{}) // Return empty array if seller not found
		return
	}

	// Then find listings for that seller with proper preloading
	var listings []models.Listing
	result := h.db.Preload("Record").Preload("Seller").
		Where("seller_id = ?", seller.ID).
		Order("score DESC").
		Find(&listings)

	if result.Error != nil {
		log.Printf("DEBUG: GORM error: %v", result.Error)
	...
	for _, listing := range listings {
		listingData := gin.H{
			...
			"record_id":        listing.RecordID,
			"record_price":     listing.RecordPrice,
			"media_condition":  listing.MediaCondition,
			"score":            listing.Score,
			"kept":             listing.Kept,
			"evaluated":        listing.Evaluated,
			"predicted_keeper": listing.PredictedKeeper,
			"seller": gin.H{
				"id":       listing.Seller.ID,
				"name":     listing.Seller.Name,
				"currency": listing.Seller.Currency,
			},
			"record": gin.H{
				"id":              listing.Record.ID,
				"discogs_id":      listing.Record.DiscogsID,
				...
				"added":           listing.Record.Added,
				"genres":          listing.Record.Genres,
				"styles":          listing.Record.Styles,
				"suggested_price": listing.Record.SuggestedPrice,
				"year":            listing.Record.Year,
			},
		}
		response = append(response, listingData)
	}

	log.Printf("DEBUG: Constructed response for %d listings", len(response))
	if len(response) > 0 {
		if record, ok := response[0]["record"].(gin.H); ok {
			log.Printf("DEBUG: First record in response: Artist=%v, Genres=%v, Styles=%v", 
				record["artist"], record["genres"], record["styles"])
		}
	}
